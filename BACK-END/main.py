...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://163.176.17.110"],
...
def analisar(request: AnaliseRequest):
    try:
        ...
        for t in request.transacoes:
            desc_segura = html.escape(t.descricao)
            cat, conf = CategoriaService.classificar(desc_segura, t.valor)
            trans_class.append({
                "descricao": desc_segura,
...
def classificar(descricao: str, valor: float):
    desc_segura = html.escape(descricao)
    cat, conf = CategoriaService.classificar(desc_segura, valor)
    return {
        "descricao": desc_segura,
        "valor": valor,
        "categoria": cat,
    ...
    try:
        ...
        for (renda, endiv, poup), gdf in grupos:
            ...
            for t in req.transacoes:
                desc_segura = html.escape(t.descricao)
                cat, conf = CategoriaService.classificar(desc_segura, t.valor)
                trans_class.append({"descricao": desc_segura, "valor": round(t.valor, 2), "categoria": cat, "confianca": round(conf, 4)})
                resumo[cat] = resumo.get(cat, 0.0) + t.valor
